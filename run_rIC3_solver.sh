...
LOG_DIR="rIC3_solver_multi_pred_logs"
mkdir -p "$LOG_DIR"
...
process_file() {
    ...
    {
        echo "File: $FILE"
        echo "Command: ~/coding_env/rIC3-exp-param/target/release/rIC3 -e ic3 --ic3-pred-samples 5 $FILE"
        echo "Started at: $(date)"
        echo "----------------------------------------"
        
        # Run the solver with timeout and capture output
        timeout 3600 ~/coding_env/rIC3-exp-param/target/release/rIC3 -e ic3 --ic3-pred-samples 5 "$FILE" 2>&1
