...
LOG_DIR="rIC3_solver_flip_high_logs"
mkdir -p "$LOG_DIR"
...
process_file() {
    ...
    {
        echo "File: $FILE"
        echo "Command: ~/coding_env/rIC3-adaptive/target/release/rIC3 -e ic3 --ic3-high-flip-rate-first --ic3-flip-rate-sort $FILE"
        echo "Started at: $(date)"
        echo "----------------------------------------"
        
        # Run the solver with timeout and capture output
        timeout 3600 ~/coding_env/rIC3-adaptive/target/release/rIC3 -e ic3 --ic3-high-flip-rate-first --ic3-flip-rate-sort "$FILE" 2>&1
